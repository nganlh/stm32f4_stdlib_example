...
void v_USART1_Init();
void v_SPI1_Init();
void v_SPI_Tx(uint8_t address, uint8_t data);
uint8_t u8_SPI_Rx(uint8_t address);
...
int main(void)
{
  ...
  while(1)
  {
    g_nZAx = u8_SPI_Rx(0x2D);    // get Z axis data
    ...
    if( ( g_nZAx & 0x80 ) == 0x80 ) // check if data is negative
    {
      ...
      v_USART_SendText("-");// add minus to note it is a negative number
      v_USART_SendNumber(g_nZAx);
    }
    else
    {
      // Is positive
